...
Każdy z tych grafów należy sprowadzić do formy kanonicznej a następnie wyznaczyć istniejące w nim orbity.

Przykładowym przekształceniem grafu $G_2$ do formy kanonicznej jest 
$$ {0,1,2} \to {2,0,1} $$
I efektem takiego przekształcenia jest graf przedstawiony na rysunku \ref{G2canon}

\begin{figure}[H]
	\centering
	\begin{tikzpicture}[node distance={15mm}, main/.style = {draw, circle}] 
   		\node[main] (2) at (0,1.5) {$1$};
   		\node[main] (0) at (2,1.5)  {$2$};
   		\node[main] (1) at (1,0)  {$3$};
    	\draw (0) -- (2);
		\draw (0) -- (1);
	\end{tikzpicture}
	\caption{Graf G2 w formie kanonicznej}
	\label{g2canon}
\end{figure} 

Orbitami tego grafu będą zbiory {0} oraz {1,2}.

Dzięku temu wiemy, że w kolejnym kroku nie trzeba podłączać nowego wierzchołka zarówno do 1 jak i do 2. Wystaczy 
zrobić to dla jednego wierzchołka w orbicie, co widać na rysunku \ref{g2extend}.
...
\end{tikzpicture}
\hfill
  \caption{Przykładowe izomorficzne rozszerzenia grafu $G_2$}
	\label{g2extend}
\end{figure} 
...
Drugą techniką użyteczną przy usuwaniu izomorfizmów jest sprowadzenie wszystkich wygenerowanych grafów danego stopnia do formu kanonicznej 
i sprawdzanie czy dodany wierchołek znajduje się w ostatniej orbicie. Jeżeli tak nie jest, to graf można odrzucić, ponieważ graf do niego
